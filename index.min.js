...
function someCollections(db, collections, next) {
    ...
    return 0 === last ? next(null) : void collections.forEach(function (collectionName) {
        const collection = db.collection(collectionName);
        void collection.drop(function (err) {
...
function wrapper(my) {
    ...
    function go(root) {
        my.metadata === !0 && (metadata = path.join(root, ".metadata", path.sep)), require("mongodb").MongoClient.connect(my.uri, my.options, function (err, client) {
            const db = client.db();
            function next(err) {
                return err ? (logger("db close"), client.close(), callback(err)) : void discriminator(db, root, metadata, parser, function (err) {
                    logger("db close"), client.close(), callback(err);
                });
            }
